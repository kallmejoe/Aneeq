...
export default defineEventHandler(async (event) => {
  ...
  try {
    ...
    if (role === "professor") {
      db.prepare("INSERT INTO professors_info (user_id) VALUES (?)").run(newUserId);
    } else if (role === "student") {
      db.prepare("INSERT INTO students_info (user_id) VALUES (?)").run(newUserId);
    }

    console.log("[signup] Success, userId:", newUserId);
